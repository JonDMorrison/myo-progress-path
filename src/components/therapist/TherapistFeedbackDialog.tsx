...
const TherapistFeedbackDialog = ({ open, onOpenChange, patientId, patientName, weekId, weekNumber, exerciseTitle, onSuccess }: TherapistFeedbackDialogProps) => {
  ...
  const uploadFile = async (file: File, type: "video" | "photo", folderId: string): Promise<string> => {
    ...
    const { error } = await supabase.storage.from("therapist-feedback").upload(fileName, file, { upsert: true });
  ...
  const handleSubmit = async () => {
    ...
    if (!(await canMessagePatient(patientId))) {
      toast({ title: "Self-guided plan", description: "This patient is on a self-guided plan and does not receive therapist feedback.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      const getContextLabel = () => {
        ...
        if (weekNumber) return `for Module ${Math.ceil(weekNumber / 2)}`;
        return '';
      };
      ...
      const isRlsRejection = error?.code === "42501" || /row-level security|violates row-level/i.test(error?.message || "");
      const supabaseDetail = [error?.code, error?.details, error?.hint].filter(Boolean).join(" — ");
      ...
      const description = isRlsRejection
        ...
        : baseMessage;
      toast({ title: "Error sending feedback", description, variant: "destructive" });
    } finally {
      setUploading(false);
    }
  };

  const context = exerciseTitle ? `for "${exerciseTitle}"` : weekNumber ? `for Module ${Math.ceil(weekNumber / 2)}` : "";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Send personalized feedback to {patientName} {context}</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4 max-h-[60vh] overflow-y-auto">
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {FEEDBACK_OPTIONS.positive.map((option) => (
                <Badge key={option} variant={selectedOptions.includes(option) ? "default" : "outline"} className={`cursor-pointer transition-all ${selectedOptions.includes(option) ? "bg-green-600 hover:bg-green-700" : "hover:bg-green-50 border-green-200"}`} onClick={() => toggleOption(option)}>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {FEEDBACK_OPTIONS.corrective.map((option) => (
                <Badge key={option} variant={selectedOptions.includes(option) ? "default" : "outline"} className={`cursor-pointer transition-all ${selectedOptions.includes(option) ? "bg-amber-600 hover:bg-amber-700" : "hover:bg-amber-50 border-amber-200"}`} onClick={() => toggleOption(option)}>
                  {selectedOptions.includes(option) && <Check className="h-3 w-3 mr-1" />}
                  {option}
                </Badge>
              ))}
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {FEEDBACK_OPTIONS.instructional.map((option) => (
                <Badge key={option} variant={selectedOptions.includes(option) ? "default" : "outline"} className={`cursor-pointer transition-all ${selectedOptions.includes(option) ? "bg-blue-600 hover:bg-blue-700" : "hover:bg-blue-50 border-blue-200"}`} onClick={() => toggleOption(option)}>
                  {selectedOptions.includes(option) && <Check className="h-3 w-3 mr-1" />}
                  {option}
                </Badge>
              ))}
          ...
          {selectedOptions.length > 0 && <p className="text-sm text-muted-foreground">{selectedOptions.length} option{selectedOptions.length > 1 ? "s" : ""} selected</p>}

          <div className="space-y-2 pt-2 border-t">
            <Label htmlFor="feedback">Additional Message (optional)</Label>
            <Textarea id="feedback" placeholder="Add custom feedback..." value={feedbackText} onChange={(e) => setFeedbackText(e.target.value)} rows={3} />
          </div>
          ...
          <div className="space-y-2">
            ...
            {videoFile ? (
              <div className="flex items-center gap-2 p-3 bg-muted rounded-lg">
                ...
                <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => setVideoFile(null)}><X className="h-4 w-4" /></Button>
              </div>
            ) : (
              <Button variant="outline" className="w-full" onClick={() => videoInputRef.current?.click()}><Video className="h-4 w-4 mr-2" />Attach Video</Button>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            {photoFile ? (
              <div className="flex items-center gap-2 p-3 bg-muted rounded-lg">
                ...
                <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => setPhotoFile(null)}><X className="h-4 w-4" /></Button>
              </div>
            ) : (
              <Button variant="outline" className="w-full" onClick={() => photoInputRef.current?.click()}><Image className="h-4 w-4 mr-2" />Attach Photo</Button>
            )}
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={uploading || (selectedOptions.length === 0 && !feedbackText.trim() && !videoFile && !photoFile)}>
            {uploading && <Loader className="h-4 w-4 animate-spin mr-2" />}
            <Upload className="h-4 w-4 mr-2" />Send Feedback
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
