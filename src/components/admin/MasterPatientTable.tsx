...
export const MasterPatientTable = ({ patients, onExport, onRefresh }: MasterPatientTableProps) => {
  ...
  const unassignedCount = useMemo(() => patients.filter(p => !p.therapist_id).length, [patients]);
  ...
  useEffect(() => {
    ...
    const loadDuplicates = async () => {
      try {
        ...
        setDuplicateNames(new Set(Object.entries(counts).filter(([, c]) => c > 1).map(([n]) => n)));
      } catch (err) {
        console.warn('MasterPatientTable duplicate-name query failed', err);
      }
    };
    loadDuplicates();
  ...
  const isDuplicate = (name: string | null | undefined) => !!name && duplicateNames.has(name.trim().toLowerCase());
  ...
  const handleToggleVideo = async (patientId: string, value: boolean) => {
    const { error } = await supabase.from('patients').update({ requires_video: value } as any).eq('id', patientId);
  ...
  const handleChangePathway = async (patientId: string, variant: string) => {
    const { error } = await supabase.from('patients').update({ program_variant: variant } as any).eq('id', patientId);
    if (error) toast.error('Could not update pathway');
    else {
    ...
    <div className="flex items-start gap-2">
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-1.5 font-medium">
          ...
          {isDuplicate(patient.patient_name) && <span title="Possible duplicate account — another patient has the same name" aria-label="Possible duplicate account" className="inline-flex"><UsersIcon className="h-3.5 w-3.5 shrink-0 text-warning" /></span>}
        </div>
  ...
  const renderPathwaySelect = (patient: MasterPatientListItem) => (
    <select value={pathways[patient.patient_id] || patient.program_variant || 'frenectomy'} onChange={(e) => handleChangePathway(patient.patient_id, e.target.value)} className="text-xs border rounded px-2 py-1 bg-background">
    ...
    <div className="space-y-4">
      {unassignedCount > 0 && (
        <Alert variant="destructive">
          ...
          <AlertTitle>{unassignedCount} unassigned patient{unassignedCount === 1 ? '' : 's'}</AlertTitle>
          <AlertDescription>Patients without a therapist still show up in Needs Review flagged as "Unassigned", but they won't belong to any therapist's workload until you assign one below.</AlertDescription>
        </Alert>
      )}

      <div className="flex justify-between items-center">
        <p className="text-sm text-muted-foreground">Showing {patients.length} patients</p>
        {onExport && <Button onClick={onExport} variant="outline" size="sm"><Download className="mr-2 h-4 w-4" />Export CSV</Button>}
      </div>
      ...
      <div className="hidden md:block border rounded-lg overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {patients.length === 0 ? (
              <TableRow><TableCell colSpan={9} className="text-center text-muted-foreground py-8">No patients found</TableCell></TableRow>
            ) : (
              patients.map((patient) => (
                <TableRow key={patient.patient_id}>
                  <TableCell>{renderPatientName(patient)}</TableCell>
