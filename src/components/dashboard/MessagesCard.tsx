...
export function MessagesCard({ messages, onSendMessage, canSend = true }: MessagesCardProps) {
            ...
            <div className="space-y-3">
              {recentMessages.map((msg) => (
                ...
                >
                  ...
                  <p className={`leading-relaxed text-xs font-medium whitespace-pre-line ${msg.sent_by === 'system' ? 'text-blue-700' : 'text-slate-600'}`}>
                    {cleanFeedbackBody(msg.body)}
                  </p>
                  <FeedbackMediaButtons videoUrl={msg.video_url} photoUrl={msg.photo_url} className="mt-2" />
                </div>
              ))}
            </div>
          )}
