...
const MAX_VIDEO_SIZE = 500 * 1024 * 1024; // 500MB cap
...
export function validateVideoFile(file: File): { valid: boolean; error?: string } {
  ...
  if (file.size > MAX_VIDEO_SIZE) {
    ...
    return { 
      valid: false, 
      error: `File size (${sizeMB}MB) exceeds the 500MB limit. Please trim your video or use a lower resolution.`
    };
  }
  ...
  try {
    ...
    const { data: insertData, error: insertError } = await supabase
      ...
      .insert({
        ...
        exercise_key: exerciseId?.startsWith('json-') ? exerciseId : null,
        week_key: weekId?.startsWith('json-') ? weekId : null,
        kind,
        file_url: fileUrl,
      })
      .select('id')
