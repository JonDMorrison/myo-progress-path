-- Migration: 4_variant_access_codes_support
-- Description: Updates calc_week_progress to respect the 4 program variants.
--   - frenectomy        : Frenectomy, NO video submissions required
--   - frenectomy_video  : Frenectomy, WITH video submissions required
--   - non_frenectomy    : Non-Frenectomy, NO video submissions required
--   - non_frenectomy_video: Non-Frenectomy, WITH video submissions required
--
-- Key changes:
--   1. Video upload check now only runs when patient's variant ends with '_video'
--   2. Frenectomy consult check now includes 'frenectomy_video' variant as well
...
DECLARE
  ...
  target_met BOOLEAN;
  -- NEW: Derived flags from program_variant
  patient_requires_video BOOLEAN;
  patient_is_frenectomy BOOLEAN;
BEGIN
  -- Get week details
  ...
  -- Derive variant flags
  -- patient_requires_video: true only if variant ends with '_video'
  patient_requires_video := (patient_record.program_variant LIKE '%_video');
  -- patient_is_frenectomy: true if variant starts with 'frenectomy'
  patient_is_frenectomy  := (
    patient_record.program_variant = 'frenectomy'
    OR patient_record.program_variant = 'frenectomy_video'
    OR patient_record.program_variant = 'standard'
  );
  ...
  -- Check video uploads only if BOTH:
  --   a) the week requires it (week_record.requires_video_first/last), AND
  --   b) the patient's variant includes video (patient_requires_video = TRUE)
  IF patient_requires_video AND week_record.requires_video_first THEN
    total_required := total_required + 1;
  ...
  IF patient_requires_video AND week_record.requires_video_last THEN
    total_required := total_required + 1;
    SELECT COUNT(*) INTO upload_count 
    FROM uploads 
    WHERE patient_id = _patient_id 
  ...
  -- Check frenectomy consult for Week 1 - applies to ALL frenectomy variants
  IF week_record.number = 1 AND patient_is_frenectomy THEN
    total_required := total_required + 1;
  ...
  -- Check learn hub reviewed for Week 1
  ...
  -- Check all exercises for this week
...
GRANT EXECUTE ON FUNCTION public.calc_week_progress TO authenticated;
